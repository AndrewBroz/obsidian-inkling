...
import { debounce, Menu, TFile } from "obsidian";
...
const range_filters = [
	...
	{
		icon: "plus-circle",
		tooltip: "Addition markup",
		value: SuggestionTypeFilter.ADDITION,
	},
	{
		icon: "minus-square",
		tooltip: "Deletion markup",
		value: SuggestionTypeFilter.DELETION,
	},
	{
		icon: "replace",
		tooltip: "Substitution markup",
		value: SuggestionTypeFilter.SUBSTITUTION,
	},
	{
		icon: "highlighter",
		tooltip: "Highlight markup",
		value: SuggestionTypeFilter.HIGHLIGHT,
	},
	{
		icon: "message-square",
		tooltip: "Comment markup",
		value: SuggestionTypeFilter.COMMENT,
	},
];
...
const content_filters = [
	...
	{
		icon: "square",
		tooltip: "Suggestions with content",
		value: ContentFilter.CONTENT,
	},
	{
		icon: "box-select",
		tooltip: "Empty suggestions",
		value: ContentFilter.EMPTY,
	},
];
...
const author_filters = [
	...
	{
		icon: "user-x",
		tooltip: "Others' suggestions",
		value: AuthorFilter.OTHERS,
	},
];
...
let description_blurb = $derived.by(() => {
	return `${filtered_items.length} ${filter_names[range_type_filter]} in the ${
		location_filters[active_file ? location_filter : LocationFilter.VAULT]
			.tooltip.toLowerCase()
	}`;
});
...
async function handleThreadClick(
	e: MouseEvent,
	...
	if (e.shiftKey) {
		...
		selected_ranges = selected_ranges.filter(
			(item) =>
				!(item.path === selected_item.path &&
					item.range === selected_item.range),
		);
...
async function handleThreadDblClick(
	e: MouseEvent,
	row: CriticMarkupRangeEntry,
	index: number,
) {
...
async function handleKey(e: KeyboardEvent) {
	if (e.key === "z" && e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey) {
		...
		e.key === "a" && e.ctrlKey && !e.shiftKey && !e.altKey && !e.metaKey
	) {
		e.preventDefault();
						...
						onClick={(evt) => {
							...
							menu.addItem((item) => {
								item
									...
									.setSection("selection-actions")
									.onClick(() => {
										const sorted_ranges = [...selected_ranges].sort((a, b) => {
										...
										for (const [idx, row] of filtered_items.entries()) {
											if (
												current_idx < sorted_ranges.length &&
												row.path === sorted_ranges[current_idx].path &&
												row.range.from === sorted_ranges[current_idx].range.from
											) {
												current_idx++;
											} else {
