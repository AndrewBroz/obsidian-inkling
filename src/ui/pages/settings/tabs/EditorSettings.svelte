<script lang="ts">
	import { Dropdown, SettingItem, Slider, Toggle } from "../../../components";
...
let comment_rendering_mode_notices = {
	...
	icon:
		"Comments are displayed as icons and can be hovered over to reveal the comment",
	none:
		"Comments are not rendered in the editor, use this with the annotation gutter",
};
</script>
	...
	notices={[
		{
			type: "info",
			text:
				"When typing <code>{++</code>, the tag automatically gets completed with <code>++}</code>",
		},
	]}
	type="toggle"
>
	...
	notices={[
		{
			type: "info",
			text:
				"Copying <code>my {++text++}</code> will result in <code>my text</code>",
		},
	]}
	type="toggle"
>
	{#snippet control()}
		<Toggle
			...
			onChange={() => {
				plugin.settings.clipboard_remove_syntax = !plugin.settings
					.clipboard_remove_syntax;
				plugin.saveSettings();
			}}
		/>
	{/snippet}
