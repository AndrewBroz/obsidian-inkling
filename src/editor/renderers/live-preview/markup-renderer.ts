...
const rangeMarkupDecoration = (range: CriticMarkupRange, cls: string = "", show_styling: boolean = false) =>
	Decoration.mark({
		attributes: {
...
function markContents(
	...
	if (left === 0) {
		if (!range.empty()) {
			decorations.push(
				rangeMarkupDecoration(range, cls, show_styling)
		...
		update(oldSet: DecorationSet, tr: Transaction) {
			...
			if (tr.docChanged || (!tr.docChanged && parsed_cm_ranges.inserted_ranges.length)) {
				...
				return oldSet.update({
					filter: (from, to, _) => {
						while (
							range_filter_idx < parsed_cm_ranges.deleted_ranges.length &&
							parsed_cm_ranges.deleted_ranges[range_filter_idx].to <= from
						) {
							range_filter_idx++;
						}
			...
			if (tr.newSelection !== tr.startState.selection) {
				...
				const cm_ranges_in_selections = parsed_cm_ranges.ranges.ranges_in_intervals(
					all_selection_ranges.ranges as unknown as {
						from: number;
						to: number;
					}[],
				);
				...
				return oldSet.map(tr.changes).update({
					filter: (from, to, _) => {
						while (
							range_filter_idx < cm_ranges_in_selections.length && cm_ranges_in_selections[range_filter_idx].to <= from
						) {
							range_filter_idx++;
						}
